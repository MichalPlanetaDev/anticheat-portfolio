...
impl TelemetryWriter {
    ...
    pub fn write_events(&mut self, events: &[TelemetryEvent]) -> io::Result<()> {
        for event in events {
...
mod tests {
    use super::*;
    use ac_protocol::{PlayerId, PlayerSnapshot, TelemetryEvent, Vec2};

    #[test]
    fn writes_and_reads_jsonl_events() {
        let path =
            std::env::temp_dir().join(format!("ac-telemetry-test-{}.jsonl", std::process::id()));
...

        {
            ...
            writer.flush().expect("failed to flush telemetry");
        }
